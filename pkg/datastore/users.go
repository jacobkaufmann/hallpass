...
func (s *usersStore) Get(id int64) (*hallpass.User, error) {
	...
	if err := s.dbh.Select(&users, `SELECT * FROM faculty WHERE user_id=$1;`, id); err != nil {
		return nil, err
	}
	if len(users) == 0 {
...
func (s *usersStore) Create(u *hallpass.User) (bool, error) {
	...
	err := transact(s.dbh, func(tx modl.SqlExecutor) error {
		...
		if err := tx.Select(&existing, `SELECT * FROM faculty WHERE email=$1 LIMIT 1;`, u.Email); err != nil {
			return err
		}
		if len(existing) > 0 {
			*u = *existing[0]
